...
const globalStore = globalThis as typeof globalThis & {
  __attakamoshirenaiDailyStore?: Map<string, RateRecord>;
};
...
const dailyStore = globalStore.__attakamoshirenaiDailyStore;
...
function extractJson(text: string): WorldResult {
  const cleaned = text
    .replace(/```json/g, "")
    .replace(/```/g, "")
    .trim();
